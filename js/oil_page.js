...
d3.select('#oil_up_stock_name').on('change', function() {

    d3.select("#oil_line_box").selectAll("div").remove()
    d3.select("#oil_line").selectAll("svg").remove()
    let upSelect = document.getElementById("oil_up_stock_name");
    let upValue = upSelect.options[upSelect.selectedIndex].value;
    checkeboxLinePlot(upValue);
})
checkeboxLinePlot("1301.TW");


function checkeboxLinePlot(upValue) {
    // 選單
    let boxLSTM = d3.select("#oil_line_box").append('div').attr("id","oil_LSTM")
                    .style("font-size", "20px").style("font-weight", "bold").append("br")

    //.style("display","inline").style("width","100%");
    //d3.select("#line_box").append("br")

    let boxGRU = d3.select("#oil_line_box").append('div').attr("id","oil_GRU")
                    .style("font-size", "20px").style("font-weight", "bold").append("br")
    //.style("display","inline").style("width","100%");
    ...
    d3.csv(`data/Line_Chart_Data/${upValue}.csv`,type).then( data => {
        ...
        for (let i = 1; i < stocks.length; i++) {
            ...
            if ((i%2) !== 0){
                ...
                document.getElementById("oil_LSTM").appendChild(divcheck);
            } else{
                var divcheck = document.createElement('div');
                divcheck.id="model";
                // tick.appendChild(document.createTextNode(countries[i].id));
                divcheck.appendChild(tick);
                divcheck.appendChild(label);
                document.getElementById("oil_GRU").appendChild(divcheck);
            };

            tick.addEventListener("click", function() {

                var lineSelected = this.value;
                var svgline = d3.select(`#oil_line-${lineSelected}`);
                var textline = d3.select(`#oil_text-${lineSelected}`);
                // console.log(svgline);
...
d3.select('#oil_down_stock_name').on('change', function() {

    d3.select("#oil_twochartOfbar").selectAll("svg").remove()
    d3.select("#oil_twochartOfbar").selectAll("div").remove()
    d3.select("#oil_twochartOfline").selectAll("svg").remove()
    d3.select("#oil_twochartOfline").selectAll("div").remove()

    let downSelect = document.getElementById("oil_down_stock_name");
    let downValue = downSelect.options[downSelect.selectedIndex].value;
    towChartInteraction(downValue);
})
towChartInteraction("1301.TW");
...
function towChartInteraction(downValue) {
    ...
    const svg1 = d3.select("#oil_twochartOfbar")
                    .append("svg")
                    .attr("preserveAspectRatio", "xMidYMid meet")
    ...
    const tooltip1 = d3.select('#oil_twochartOfbar')
                        .append('div')
    ...
    const svg2 = d3.select("#oil_twochartOfline")
                    .append("svg")
                    .attr("preserveAspectRatio", "xMidYMid meet")
                    .attr("viewBox", "0 0 550 600")
                    .attr("width", width + margin.left + margin.right)
    ...
    const tooltip2 = d3.select('#oil_twochartOfline')
                    .append('div')
                    .attr('class', 'd3-tooltip')
                    .style('position', 'absolute')
                    .style('z-index', '50')
    ...
    d3.csv(`data/Single_Stock_RMSE_Bar_Data/Metrics_Score_rmse_${downValue}.csv`).then( function(data) {
        ...
        svg1.selectAll("mybar")
            ...
            d3.select(`#oil-line2chart-${tempid[2]}`).attr("opacity", 1);
            ...
            // interactive with line
            let tempid = this.id.split("-");
            d3.select(`#oil-line2chart-${tempid[2]}`).attr("opacity", 0.1);
    ...
    d3.csv(`data/Line_Chart_Data/${downValue}.csv`,type).then( data => {
        ...
        stock.append("path")
            ...
            .on("mouseover", function(d){
                ...
                if (this.id != "oil-line2chart-Historical") {
                ...
                d3.select(`#oil-bar-${tempid[2]}`).style("opacity", 1);
            ...
            .on("mouseout", function(d){
                ...
                // interactive with bar
                let tempid = this.id.split("-")
                d3.select(`#oil-bar-${tempid[2]}`).style("opacity", 0.2);
